...
//   Signing in protects the shared database (RLS requires an authenticated
//   session), so the data is genuinely access-controlled and shared live.
//
...
const LOCAL_PASSWORD = import.meta.env.VITE_APP_PASSWORD ?? "boiler-2026";

/** True when running the local static gate with its built-in default password. */
export const USING_DEFAULT_CREDENTIALS =
  !isSupabaseConfigured && !import.meta.env.VITE_APP_PASSWORD;

export const AUTH_MODE: "cloud" | "local" = isSupabaseConfigured
  ? "cloud"
  : "local";

const LOCAL_STORAGE_KEY = "boiler-auth-v1";
