...
def add_agenda_slide(prs: Presentation, counter: SlideCounter) -> None:
    ...
    top = add_header(slide, "Today's Briefing", "Where we are in a 27-slide working session")
...
def build() -> Path:
    ...
    add_bullet_slide(
        ...
        [
            "Walk every boiler room — locate each relief valve and protected point",
    ...
    add_two_column_cards(
        ...
        "Equipment & location",
        ["Unit identity (boiler / HRSG)", "Protected location on system", "Installed vs. spare status"],
        "Valve identity & condition",
        ["Serial number & manufacturer", "Set pressure, capacity, NB #", "Nameplate photo evidence"],
    )
    ...
    add_icon_row_slide(
        ...
        [
            ("📈", "Fleet KPIs", "Total, Installed, OOS, Overdue, Compliant %"),
            ("🔍", "Drill-down", "Click any KPI → valve list with Inventory ID"),
            ("🗂", "Hierarchy", "Equipment → Location → PSV faceplates"),
            ("📑", "Export", "5-sheet Excel report for meetings"),
        ],
    ...
    add_screenshot_slide(prs, counter, "Dashboard — Fleet Compliance at a Glance", "01-dashboard.png",
                         "Live production — 51 PSVs, 78% compliant, fleet-wide equipment view")
    ...
    add_icon_row_slide(
        ...
        [
            ("📦", "Spare coverage", "Acquire spares for no-spare locations"),
            ("🎯", "Criticality", "Rank valves for capital prioritization"),
            ("👀", "Walkdowns", "Semi-annual visual checks as standard"),
            ("📌", "One source", "Dashboard + Excel as system of record"),
        ],
        subtitle="Plus: management decision on routine manual lever testing",
    )
    ...
    add_two_column_cards(
        ...
        "Operational value",
        ["Complete fleet inventory", "Visible compliance by site", "Traceable audit history", "Faster Setpoint planning"],
        "Strategic value",
        ["Code-aligned strategy doc", "CMMS & spare-pool foundation", "Capital requests backed by data", "Sustainable UES ownership"],
    )
